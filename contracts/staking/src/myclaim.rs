...
use cosmwasm_std::{Addr, BlockInfo, CustomQuery, Deps, StdResult, Storage, Uint128, Order};
...
impl<'a> Claims<'a> {
    ...
    // returns the total amount of claims that have expired at a certain block and are ready to be claimed.
    pub fn query_expired_claims<Q: CustomQuery>(
        &self,
        deps: Deps<Q>,
        block: &BlockInfo,
    ) -> StdResult<Uint128> {
        let send_amount : Uint128= self.0
        .range(deps.storage, None, None, Order::Ascending)
        .map(|item| item.unwrap().1)
        .into_iter()
        .flatten()
        .filter(|item| item.release_at.is_expired(block))
        ...
        // // flat_map method is used to flatten the outer Vec of Vecs into a single Vec of Claims
        // let send_amount : Uint128= to_send
        // .into_iter()
...
mod test {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////
    #[test]
    fn test_query_claim_tokens_with_no_released_claims() {
        let mut deps = mock_dependencies();
        let claims = Claims::new("claims");
        ...
        let amount_ready_to_claim = claims.query_expired_claims(deps.as_ref(), &env.block).unwrap();

        assert_eq!(amount, Uint128::zero());
        assert_eq!(amount_ready_to_claim, Uint128::zero());
        assert_eq!(saved_claims.len(), 2);
        assert_eq!(saved_claims[0].amount, Uint128::from(TEST_AMOUNT + 100));
        assert_eq!(saved_claims[0].release_at, Expiration::AtHeight(10));
        assert_eq!(saved_claims[1].amount, Uint128::from(TEST_AMOUNT + 100));
    ...
    #[test]
    fn test_query_claim_tokens_with_one_released_claim() {
        let mut deps = mock_dependencies();
        let claims = Claims::new("claims");
        ...
        let amount_ready_to_claim = claims.query_expired_claims(deps.as_ref(), &env.block).unwrap();
        assert_eq!(amount_ready_to_claim, Uint128::from(TEST_AMOUNT));
    ...
    #[test]
    fn test_query_claim_tokens_with_all_released_claims() {
        let mut deps = mock_dependencies();
        let claims = Claims::new("claims");
        ...
        let amount_ready_to_claim = claims.query_expired_claims(deps.as_ref(), &env.block).unwrap();
        assert_eq!(amount_ready_to_claim, Uint128::from(2*TEST_AMOUNT+100));
    ...
    #[test]
    fn test_query_claim_tokens_with_some_released_claims_from_few_addrs() {
        let mut deps = mock_dependencies();
        let claims = Claims::new("claims");
        ...
        let amount_ready_to_claim = claims.query_expired_claims(deps.as_ref(), &env.block).unwrap();
        assert_eq!(amount_ready_to_claim, Uint128::from(3*TEST_AMOUNT));
